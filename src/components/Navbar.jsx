...
import puterLogo from '../assets/puter-logo.png'
    ...
    const checkAuthStatus = async () => {
      try {
        if (typeof window.puter !== 'undefined') {
          ...
          if (onAuthChange) onAuthChange(signedIn)
        }
      } catch (error) {
  ...
  const handlePuterLogin = async () => {
    try {
      if (typeof window.puter !== 'undefined') {
        ...
        // Fallback: redirect to Puter website
    ...
    <nav className="bg-github-bg-secondary border-b border-github-border px-4 py-3">
      <div className="container mx-auto flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <h1 className="text-xl font-semibold text-github-text">Git Commit Writer</h1>
        </div>
        
        <div className="flex items-center space-x-4">
          {isChecking ? (
            <div className="flex items-center space-x-2 text-github-text-secondary text-sm px-3 py-2">
              <div className="animate-spin w-4 h-4 border border-github-border border-t-github-accent rounded-full"></div>
              <span>Checking...</span>
            </div>
          ) : isSignedIn ? (
            <div className="flex items-center space-x-3">
